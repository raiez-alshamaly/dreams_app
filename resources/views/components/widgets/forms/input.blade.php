...
<div class=" mb-4 {{$divstyle}} ">
    ...
    @else
        <div class="relative">
            <input type="{{$type}}" id="{{ $id }}" name="{{ $id }}"
                aria-describedby="{{ $id }}-helper"
                {{ $attributes->merge(['class' => 'border border-gray-300 text-gray-900 sm:text-sm rounded-lg bg-cyan-50 focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5  bg-red ']) }}>
        </div>
    @endif
